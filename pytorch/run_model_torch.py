...
import LN_malignancy_GNN.pytorch.lightning_GNN as gnn_modules
from LN_malignancy_GNN.pytorch.dataset_class import DatasetGeneratorImage
from LN_malignancy_GNN.pytorch.run_model_base import RunModel
...
class RunMalignancyModel(RunModel):
    ...
    def set_train_test_split(self):
        ...
        #copying fold distribution that is in Liyuan's code
        if self.config['alternate_splitting']:
            #copying fold distribution that is in Liyuan's code
            #self.train_folds = [[1,2,3],
            #                    [0,3,4],
            #                    [1,3,4],
            #                    [0,3,4],
            #                    [0,1,4]]
            #self.val_folds = [0,1,2,2,2]
            #self.test_folds = [4,2,0,1,3]
            #copying fold distribution from my nested training
            self.train_folds = [[4,1,2],
                                [4,0,2],
                                [4,0,1],
                                [4,0,1],
                                [0,1,2]]
            self.val_folds = [3, 3, 3, 2, 3]
